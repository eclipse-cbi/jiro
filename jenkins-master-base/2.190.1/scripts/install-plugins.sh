...
# JENKINS_PLUGIN_REF: directory with preinstalled plugins. Default: /usr/share/jenkins/ref/plugins
...
: "${JENKINS_PLUGIN_REF:="${JENKINS_REF}/plugins"}"
FAILED="${JENKINS_PLUGIN_REF}/failed-plugins.txt"

. /usr/local/bin/jenkins-support

getLockFile() {
    printf '%s' "${JENKINS_PLUGIN_REF}/${1}.lock"
}

getArchiveFilename() {
    printf '%s' "${JENKINS_PLUGIN_REF}/${1}.jpi"
}
...
installedPlugins() {
    for f in "${JENKINS_PLUGIN_REF}"/*.jpi; do
...
main() {
    ...
    mkdir -p "${JENKINS_PLUGIN_REF}" || exit 1
    rm -f "$FAILED"
