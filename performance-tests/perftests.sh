...
kubectl --context="${CONTEXT}" get nodes -l "${NODE_ROLE},role!=storage_node" -o json | jq -r '.items[]|select(.spec.unschedulable != true)|.metadata.labels["kubernetes.io/hostname"]' | \
